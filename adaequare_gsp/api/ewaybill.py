...
def generate_ewaybill(dt, dn, dia, company_gstin):
    ...
    log_values = {
        "ewaybill": ewaybill,
        "ewaybill_date": datetime.strptime(result.get("ewayBillDate"), DATE_FORMAT),
    ...
    publish_progress(percent=100, **progress)
...
def cancel_ewaybill(dt, dn, dia, company_gstin):
    ...
    publish_progress(percent=33, **progress)
...
def validate_invoice_for_ewaybill(doc, method=None):
    ...
    if doc.get("lr_date") and doc.lr_date < doc.posting_date:
        doc.lr_date = doc.posting_date

    if doc.get("distance", 0) > 4000:
        doc.distance = 4000
...
def validate_state_tax(data):
    attribute, supply = (
        ...
        else (("cgst", "sgst"), "inter")
    )

    for attr in attribute:
        if (
            data.get(attr + "Value") == 0
            or data.get("itemList")[0].get(attr + "Rate") == 0
        ):
            continue
        frappe.throw(
            "Please check the GST Accounts. {} should be zero for {}-state transaction.".format(
                attr.upper(), supply
            )
        )

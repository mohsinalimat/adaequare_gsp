  ...
  frappe.ui.form.on(dt, {
      ...
      frappe.model.get_value(
        ...
        function (d) {
          ...
          if (
            ...
            (adaequare_gsp.get_date(frm.doc.eway_bill_validity) <
              now.addHours(8) ||
              adaequare_gsp.get_date(frm.doc.eway_bill_validity).addHours(8) >
                now)
          ) {
    ...
    before_cancel: function (frm) {
      if (!frm.doc.allow_cancel && frm.doc.ewaybill_json) {
        ...
        if (
          adaequare_gsp.get_date(ewaybill_json.ewayBillDate).addHours(24) > now
        ) {
          warn_before_cancel(
            'Ewaybill has been created against this Invoice. <b>Cancel the ewaybill before cancelling invoice</b> if you are making changes that will affect you ewaybill.'
          );
        } else {
          warn_before_cancel(
            'Ewaybill has been created against this Invoice and cannot be cancelled. You should not make changes to this invoice that affects ewaybill data.'
          );
        }
      }
    }
...
adaequare_gsp.reprint_ewaybill = function (frm) {
  ...
  frappe.call({
    ...
    callback: function (r) {
      frm.reload_doc();
      frappe.msgprint(
        __('Ewaybill Print generated successfully and attached here.')
      );
    }
  });
};
    ...
    fields: [
      {
        ...
        onchange: function (e) {
          adaequare_gsp.get_gst_tranporter_id(
            cur_dialog.fields_dict.transporter.value
          );
        }
      },
      {

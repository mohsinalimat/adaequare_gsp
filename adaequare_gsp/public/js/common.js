...
adaequare_gsp.autofill_address = function (dia, pincode) {
    ...
    if (pincode.length != 6 || !adaequare_gsp.gstin_info.all_address)
        return;
...
adaequare_gsp.pincode_awesomplete = function (dia) {
    const field = dia.is_dialog? dia.fields_dict.pincode_custom: dia.fields_dict.pincode;
    ...
    field.$input
        .on("input", function (e) {
            e.target.awesomplete.list =
                adaequare_gsp.gstin_info.all_address.map((a) => a.pncd);
        })
		.on("awesomplete-open", () => {
    ...
    let party_fields = [
        {
            ...
            label: "Customer",
			options: "Customer",
			default: !cur_frm ? '' : cur_frm.doc.doctype == "Customer" ? cur_frm.doc.name : cur_frm.doc.customer ? cur_frm.doc.customer : '',
            onchange: function () {
        ...
        },
        {
            ...
            label: "Supplier",
			options: "Supplier",
			default: !cur_frm ? '' : cur_frm.doc.doctype == "Supplier" ? cur_frm.doc.name : cur_frm.doc.supplier ? cur_frm.doc.supplier : '',
            onchange: function (e) {
    ...
    let gstin_fields = [
        {
            ...
            fieldtype: "Data",
            description:
                "Autofill party information by entering correct GSTIN.",
            onchange() {
...
adaequare_gsp.get_variant_fields = function (dia) {
    ...
    variant_fields = variant_fields.concat(
        adaequare_gsp.get_address_fields(dia)
    );

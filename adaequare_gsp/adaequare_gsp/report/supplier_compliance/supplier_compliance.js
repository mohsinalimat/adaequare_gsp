...
frappe.query_reports["Supplier Compliance"] = {
	"filters": [
		{
			...
			async on_change(report) {
				...
				const return_period = report.report_settings.fiscal_years_map[fiscal_year];
	...
	async set_global_variables() {
		this.fiscal_years = await frappe.db.get_list(
			"Fiscal Year",
			{
	...
	set_defaults(report) {
		...
		report.set_filter_value('return_period', this.fiscal_years_map[this.default_fiscal_year]);

	},
};
async function fetch_latest_returns(report) {
	const { message } = await frappe.call({
		method: "adaequare_gsp.adaequare_gsp.report.supplier_compliance.supplier_compliance.fetch_latest_returns",
		args: {

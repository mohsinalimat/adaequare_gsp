...
def enqueue_bulk_update_party(make_urp):
    ...
    frappe.msgprint(
        "Verify customers and suppliers after some time.", title="Bulk Update started"
    )
    for dt in doctypes:
        parties = get_parties(dt)
        frappe.enqueue(
            update_party_gstin_details, make_urp=make_urp, dt=dt, parties=parties
        )
...
def update_address(address, gstin, gstin_info):
    for addr in address:
        ...
        frappe.db.set_value(
            "Address",
            addr.name,
            {"gst_category": gst_category, "gstin": gstin_info.get("gstin")},
        )
...
def get_parties(dt, filters=None):
    fields = ["name", "default_gstin", "pan"]
    if dt == "Supplier":
        fields.append("is_transporter")

    return frappe.db.get_all(dt, fields=fields, filters=filters)
